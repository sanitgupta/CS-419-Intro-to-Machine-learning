...
class DataLoader(object):
    ...
    # __getitem__ handles integer based indexing of the object 
...
#        self.data=data
        ...
        input_data = np.hstack([np.ones((len(input_data), 1), dtype=np.int32), input_data])
...
def train(data_loader, loss_type, regularizer_type, loss_weight):
    ...
    for i in range(num_epochs):
        ...
        for j in range(len(data_loader)):
            ...
            objective_function = get_objective_function(trainx,trainy,loss_type, regularizer_type,loss_weight)
            gradient_function = get_gradient_function(trainx,trainy, loss_type, regularizer_type, loss_weight)
    ...
    plt.plot(lossL)
    plt.title('Compi1 ' + args.loss_type + ' Regularization = ' + args.regularizer_type +  ' C =' + str(args.loss_weight))
    plt.show()
...
def get_data(data_file):
    ...
    data = data[1:]
    input_data = np.array([[float(col) for col in row.split(',')] for row in data])
    input_data= input_data[:, 1:]
    input_data = np.hstack([np.ones((len(input_data), 1), dtype=np.float32), input_data])
...
#    d=DataLoader('train.csv',22)
    ...
    parser.add_argument("--batch-size", action="store", dest="batch_size", type=int, help="Batch size for training", default=510)
    ...
    parser.add_argument("--loss-weight", action="store", dest="loss_weight", type=float, help="Relative weight", default=1)  
